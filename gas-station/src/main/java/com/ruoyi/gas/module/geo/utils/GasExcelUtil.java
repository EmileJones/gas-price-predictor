...
public class GasExcelUtil<T> {
     ...
     * @param list      数据
     */
    public void createSheet(String sheetName, List<T> list) {
    ...
    private T parseData(Row row) {
        ...
        for (int i = 0, length = row.getPhysicalNumberOfCells(); i < length; i++) {
    ...
    public static final boolean isNullSheet(Sheet sheet) {
        return sheet == null;
    }

    public static final boolean isRowEmpty(Row row) {
        if (row == null) {
            return true;
        }
